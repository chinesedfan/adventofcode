...
function solve(part, data) {
	...
	while (stack.length) {
		...
		if (children.length) {
			...
			do {
				console.log(node.player, node.boss, node.shieldTimer, node.poisonTimer, node.rechargeTimer);
				console.log(node.spell);
...
State.prototype.children = function() {
	...
	return _.chain(ret).map(function(spell) {
		...
		return !child.winner || child.winner == 'player';
	}).value();
};
