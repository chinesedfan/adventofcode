        ...
        for (let row = 0; row < limit; row++) {
            for (let col = 0; col < limit; col++) {
                ...
                for (let i = 0; i < G_SIZE; i++) {
                    finalGrid[row * G_SIZE + i] = finalGrid[row * G_SIZE + i] || []
                    for (let j = 0; j < G_SIZE; j++) {
                        finalGrid[row * G_SIZE + i][col * G_SIZE + j] = grid[i][j]
                    }
                }
            }
        }
...
function transformGrid(grid, isr, r) {
    ...
    for (let i = 0; i < G_SIZE; i++) {
        for (let j = 0; j < G_SIZE; j++) {
            ...
            switch (r) {
            case 1:
                finalGrid[j][G_SIZE - 1 - i] = grid[i][j]
                break
            case 2:
                finalGrid[G_SIZE - 1 - i][G_SIZE - 1 - j] = grid[i][j]
                break
            case 3:
                finalGrid[G_SIZE - 1 - j][i] = grid[i][j]
                break
            case 0:
            default:
