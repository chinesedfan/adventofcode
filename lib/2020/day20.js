...
module.exports = function(part, data) {
    ...
    if (part == 1) {
        ...
        const r3 = /.#..#..#..#..#..#.../
        let count = 0
        const lines = finalGrid.map(x => x.join('').replace(/\./g, ' '))
        lines.forEach((line, row) => {
            let matches
            while (matches = r2.exec(line)) {
                const found = matches && row
                    && r1.test(lines[row - 1].slice(matches.index, matches.index + LEN))
                    && r3.test(lines[row + 1].slice(matches.index, matches.index + LEN))
                if (found) {
                    console.log('good', row, matches.index)
                    count++
                }
            }
        })
