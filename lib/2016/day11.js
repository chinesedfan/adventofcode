'use strict';
const _ = require('lodash');

module.exports = function(part, data) {
    let count = 0;
    const floors = _.map(data.split('\n'), (line) => {
        const arr = [];
        line.replace(/(\S+)( generator|(?:-compatible microchip))/g, (match, key, type) => {
            arr.push({
                key,
                type: type == ' generator' ? 'G' : 'M'
            });
            count++;
        });
        return arr;
    });

    let q = [{
        steps: 0,
        elevator: 0,
        floors
    }];
    while (q.length) {
        const state = q.pop();
        const nextStates = getNextStates(state, count);
        // special flag
        if (nextStates == -1) {
            console.log(state.steps + 1);
            break;
        }
        q = q.concat(nextStates);
    }
};

function getNextStates(state, count) {
    let nextStates = [];
    // move down
    if (state.elevator) {
        nextStates = nextStates.concat(
            _.map(getNextFloors(state.floors, state.elevator, state.elevator - 1), (floors) => {
                return {
                    steps: state.steps + 1,
                    elevator: state.elevator - 1,
                    floors
                };
            })
        );
    }
    // move up
    let done = false;
    if (state.elevator != state.floors.length - 1) {
        nextStates = nextStates.concat(
            _.map(getNextFloors(state.floors, state.elevator, state.elevator + 1), (floors) => {
                if (floors[floors.length - 1].length == count) done = true;
                return {
                    steps: state.steps + 1,
                    elevator: state.elevator + 1,
                    floors
                };
            })
        );
    }
    return done ? -1 : nextStates;
}
function getNextFloors(floors, index1, index2) {
    const fromItems = floors[index1];
    const toItems = floors[index2];

    let nextFloors = [], f1, f2;
    _.each(fromItems, (item1) => {
        // take 1
        f1 = _.filter(fromItems, (item) => item.key != item1.key || item.type != item1.type);
        f2 = toItems.concat([item1]);
        updateNextFloors();

        // take 2
        _.each(fromItems, (item2) => {
            if (item1 == item2) return;

            f1 = _.filter(f1, (item) => item.key != item2.key || item.type != item2.type);
            f2 = f2.concat([item2]);
            updateNextFloors();
        });
    });

    function updateNextFloors() {
        if (!validFloor(f1) || !validFloor(f2)) return;

        nextFloors.push(_.map([].concat(floors), (items, i) => {
            if (i == index1) return f1;
            if (i == index2) return f2;
            return items;
        }));
    }

    return nextFloors;
}

function validFloor(items) {
    let hasG = false;
    const map = {};
    _.each(items, (item) => {
        if (item.type == 'G') {
            hasG = true;
        }
        map[item.key] = (map[item.key] || '') + item.type;
    });
    return !hasG || _.every(map, (c) => c != 'M');
}
