...
function f2(numbers, lines) {
    ...
    // R,6,R,8,R,8,
    // L,10,L,8,R,8,L,8,R,6
    const A = 'L,10,L,8,R,8,L,8,R,6';
    const B = 'R,6,R,8,R,8';
    const C = 'R,6,R,6,L,8,L,10';
    const main = 'A,A,B,C,B,C,B,C,B,A';
    ...
    const g = run([...numbers]);
    let a = g.next();
    // requires about 7s in Mac Pro 2015, about 170k instructions
    do {
        if (a.value === 'input') {
            console.log('rest input length:', inputs.length)
