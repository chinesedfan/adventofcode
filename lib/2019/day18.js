module.exports = function(part, data) {
    ...
    if (part == 1) {
        ...
        const rnode = rs[0];
        rs[0] = {r: -1, c: -1};
        bfs(rnode, grid, ks.filter(x => /[a-z]/.test(x.ch)).length, rs);
    }
};
...
function bfs(rnode, map, limit, irs) {
    ...
    while (q.length) {
        ...
        const ps = p.split('');
        const last = ps.pop();
        const key = ps.sort().concat([last]).join('') + getCacheKey(rs);
        if (cache[key] && cache[key] <= d) continue;
        cache[key] = d;
