...
module.exports = function(part, data) {
    ...
    bfsWithNode({k: 'o-AA'}, table, distance, ns, part == 2 ? 1 : 0);
    return;
...
function bfsWithNode(rn, table, distance, ns, delta) {
    ...
    const visited = {};
    while (q.length) {
        const {k, l, d} = q.shift(); // x as well as r, y as well as c
        ...
        for (let k2 in table[k]) {
            ...
            if (k === k2 || visited[key(k2, nl)]) continue;
