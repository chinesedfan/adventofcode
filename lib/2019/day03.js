        ...
        line2.forEach((p2) => {
            ...
            if (p1.ish && p2.ish) {
                ...
                if (ox) {
                    ...
                    p.s1 = p1.step + Math.abs(p.x - p1.x);
                    p.s2 = p2.step + Math.abs(p.x - p2.x);
                }
            } else if (p1.ish) {
                if (isMiddle(p1.y, p2.y, p2.ey) && isMiddle(p2.x, p1.x, p1.ex)) {
                    ...
                    p.s1 = p1.step + Math.abs(p.x - p1.x);
                    p.s2 = p2.step + Math.abs(p.y - p2.y);
                }
            } else if (p2.ish) {
                if (isMiddle(p1.x, p2.x, p2.ex) && isMiddle(p2.y, p1.y, p1.ey)) {
                    ...
                    p.s1 = p1.step + Math.abs(p.y - p1.y);
                    p.s2 = p2.step + Math.abs(p.x - p2.x);
                }
            } else {
            ...
            if (p && (p.x || p.y)) {
                min = Math.min(min, part == 1 ? Math.abs(p.x) + Math.abs(p.y) : p.s1 + p.s2);
            }
        });
    });
