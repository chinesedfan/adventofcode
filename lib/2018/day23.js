...
module.exports = function(part, data) {
    ...
    if (part === 1) {
        ...
        cube = _.maxBy(
            ...
            (p1) => max.map((p2) => inRange(p2, p1)).filter((b) => b).length
        );
...
function cubeInRange(p, cube) {
    return p.r + cube.r >= Math.abs(p.x - cube.x)
        && p.r + cube.r >= Math.abs(p.y - cube.y)
        && p.r + cube.r >= Math.abs(p.z - cube.z);
}
...
function getSubCubes(cube) {
    const {x, y, z} = cube;
    const r1 = Math.floor(cube.r / 2);
    const r2 = Math.ceil(cube.r / 2);
    // sorted by nearest to farest, based on (0, 0, 0)
    return _.sortBy([
        {x: x - r1, y: y - r1, z: z - r1, r: r1},
        {x: x - r1, y: y + r1, z: z - r1, r: r1},
        {x: x + r1, y: y - r1, z: z - r1, r: r1},
        {x: x + r1, y: y + r1, z: z - r1, r: r1},
        {x: x - r2, y: y - r2, z: z + r2, r: r2},
        {x: x - r2, y: y + r2, z: z + r2, r: r2},
        {x: x + r2, y: y - r2, z: z + r2, r: r2},
        {x: x + r2, y: y + r2, z: z + r2, r: r2}
    ], (c) => c.x + c.y + c.z);
