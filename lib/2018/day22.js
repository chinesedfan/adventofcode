    ...
    while (1) {
        ...
        const hasReached = cache[t].some(({x, y, eq}) => {
            ...
            if (x == target.x && y == target.y) {
                ...
                // switch equipment at the current position
                [EQ.none, EQ.torch, EQ.gear].forEach((neq) => {
                    if (eq === neq) return;
                    checkAndPush(depth, target, grid, cache, neq, x, y, t + COST);
                });
                return false;
            }
        });
        ...
        delete cache[t]; // better to release memory
        t++;
    console.log(t, (cache[t] || []).length);
    }
    console.log(t);
}
...
function checkAndPush(depth, target, grid, cache, eq, x, y, t) {
    ...
    case 0: // rocky
        ...
        break;
    case 1: // wet
        if (eq === EQ.torch) return;
        break;
    case 2: // narrow
        if (eq === EQ.gear) return;
        break;
    }
